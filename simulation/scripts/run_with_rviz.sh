...
SIM_LOG="$ROOT_DIR/rviz/simulation.log"
mkdir -p "$ROOT_DIR/rviz"

ensure_built() {
  local install_setup="$WS_DIR/install/setup.bash"
  local install_launch="$WS_DIR/install/$PKG_NAME/share/$PKG_NAME/launch/grid_sim.launch.py"
...
cd "$WS_DIR"
exec stdbuf -oL -eL ros2 launch $PKG_NAME grid_sim.launch.py use_rviz:=true 2>&1 | tee "$SIM_LOG"
