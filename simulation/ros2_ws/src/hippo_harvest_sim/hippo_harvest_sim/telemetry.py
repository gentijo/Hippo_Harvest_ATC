...
class RunContextPublisher:
    def __init__(self, node: Node, run_id: str, robot_traceparents: dict[str, str]) -> None:
        self.node = node
        self.run_id = run_id
        ...
        self.run_id_pub = node.create_publisher(String, RUN_ID_TOPIC, qos)
    ...
    def publish(self) -> None:
        run_msg = String()
        run_msg.data = self.run_id
        self.run_id_pub.publish(run_msg)
    ...
    def _on_robot_contexts(self, msg: String) -> None:
        ...
        for robot_id, value in decoded.items():
            ...
            if isinstance(traceparent, str):
                self.robot_traceparents[str(robot_id)] = traceparent
